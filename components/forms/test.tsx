...
import { Client, UpdateClientData } from '@/utils/interfaces';
...
const UpdateClientForm = ({ clientId }: UpdateClientFormProps) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!modifiedFields.includes(e.target.name)) {
      setModifiedFields((prevFields) => [...prevFields, e.target.name]);
    }
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const modifiedData: Partial<UpdateClientData> = {};
/*
    modifiedFields.forEach((fieldName) => {
      modifiedData[fieldName] = formData[fieldName];
    });*/

    updateClient({clientId: clientId, payload: modifiedData})
      .unwrap()
      .then((result) => {
